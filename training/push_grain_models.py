#!/usr/bin/env python3
"""
Generate Kaggle notebooks for grain model training and push them to Kaggle.

Creates .ipynb notebooks from each training script, configures them for
Kaggle GPU execution, and pushes them to the vann234 account.

Three notebooks are generated:
    1. coffevision-grain-cnn-train-v1        — Custom CNN training
    2. coffevision-grain-resnet50-train-v1   — ResNet50 transfer learning
    3. coffevision-grain-efficientnet-train-v1 — EfficientNetB0 transfer learning

Usage:
    python push_grain_models.py
    python push_grain_models.py --source-dir ./training --kaggle-account vann234
"""
...
# Notebook template for Kaggle training (Broca grain dataset)
NOTEBOOK_TEMPLATE = """\
{{
 "cells": [
  {{
   ...
   "source": [
    ...
    "\\n",
    "Training {model_type} for grain classification ({num_classes} classes).\\n",
    "Dataset: Roboflow Broca Coffee Grain (Broca dataset, ~1500 images).\\n",
    "\\n",
    "**Notebook Slug**: `{slug}`\\n",
  ...
  {{
   ...
   "source": [
    ...
    "# Dataset source: Broca Coffee Grain — ~1500 images, 3 classes\\n",
    "\\n",
  ...
  {{
   ...
   "source": [
    ...
    "    --output /kaggle/working/data/grain\\n",
    "\\n",
    "# Run data_pipeline.py (grain, 3 classes)\\n",
    "!python /kaggle/working/data_pipeline.py \\\\\\n",
    "    --data-dir /kaggle/working/data/grain \\\\\\n",
    "    --dataset grain \\\\\\n",
    "    --output-dir /kaggle/working/splits/grain"
   ]
  }},
  {{
   "cell_type": "code",
   ...
   "source": [
    ...
    "    --dataset grain \\\\\\n",
    "    --data-dir /kaggle/working/data/grain \\\\\\n",
    "    --output-dir /kaggle/working/models/grain\\n",
    "\\n",
  ...
  {{
   ...
   "source": [
    ...
    "MODEL_PATH = f'/kaggle/working/models/grain/grain_{model_key}.keras'\\n",
    "\\n",
...
def generate_notebook(
    ...
    Args:
        ...
        slug: Kaggle notebook slug (e.g., 'coffevision-grain-cnn-train-v1')
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate and push grain model training notebooks to Kaggle."
    )
    parser.add_argument(
        "--source-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="vann234",
        help="Kaggle account username (default: vann234)",
    )
    parser.add_argument(
        "--kgat-token",
        type=str,
    ...
    parser.add_argument(
        "--roboflow-url",
        type=str,
        default="https://universe.roboflow.com/ds/broca-coffee-grain?key=REPLACE_ME",
        help="Roboflow dataset download URL (default: placeholder for Broca dataset)",
    )
    parser.add_argument(
        "--num-classes",
        type=int,
        default=3,
        help="Number of grain classes (default: 3)",
    )
    parser.add_argument(
        "--python-version",
        type=str,
    ...
    print(f"{'='*60}")
    print(f"Push Grain Models to Kaggle")
    print(f"{'='*60}")
    ...
    # Define the three grain model notebooks
    notebooks = [
        {
            "script": "build_custom_cnn.py",
            "name": "CoffeeVision Grain Custom CNN",
            "type": "Custom CNN Architecture",
            "key": "custom_cnn",
            "slug": "coffevision-grain-cnn-train-v1",
        },
        {
            "script": "build_resnet50.py",
            "name": "CoffeeVision Grain ResNet50",
            "type": "ResNet50 Transfer Learning",
            "key": "resnet50",
            "slug": "coffevision-grain-resnet50-train-v1",
        },
        {
            "script": "build_efficientnet.py",
            "name": "CoffeeVision Grain EfficientNet",
            "type": "EfficientNetB0 Transfer Learning",
            "key": "efficientnetb0",
            "slug": "coffevision-grain-efficientnet-train-v1",
        },
    ]
    ...
    print(f"Account:     {args.kaggle_account}")
    print(f"Dataset:     Grain (Broca, {args.num_classes} classes)")
